...
# Usage
# perl %prog sam outdir
#   sam - path to a BAM/SAM file
#   output_basename - optional, output file basename, default to sam file basename without .sam extension

my ($sam_fi, $basename) = @ARGV;
my @sam_fi_split = split(/\//,$sam_fi);
my $sam_fi_short = $sam_fi_split[$#sam_fi_split];
if (not defined $basename) {
    $basename = $sam_fi_short;
    $basename =~ s/\.[bs]am$//;
}
