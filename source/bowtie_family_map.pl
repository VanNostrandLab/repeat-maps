...
# Usage:
# perl bowtie.family.map.pl fastq bowtie_db cpus output_basename
#    fastq - path to fastq file
#    bowtie_db - path to the bowtie db (basename)
#    output - path to the output sam file
#    species - name of the species, e.g., hg19, hg38
#    cpus - maximum number of cpus can be used to init multiple processes.
...
my $repeat_element_file;
if ($species eq "hg38") {
    $repeat_element_file = "/storage/vannostrand/reference_data/repeat-family-mapping-grch38/repeat.elements.list";
} elsif ($species eq "hg19") {
    $repeat_element_file = "/storage/vannostrand/reference_data/repeat-map-hg19/repeat.elements.list";
} else {
    print STDERR "error $species not implemented yet\n";
    exit;
}

&load_repeat_element($repeat_element_file);
...
my $basename = $output;
$basename =~ s/\.sam$//;
my $bowtie_out = $basename.".bowtie.log";
open(SAMOUT,">$output");
my $multimapping_out = $basename.".multi.map.delete.tsv";
open(MULTIMAP,">$multimapping_out");

#
my $command = "bowtie2 -q --sensitive -a -p $cpus --no-mixed --reorder -x $bowtie_db -U $fastq_file1 2> $bowtie_out";
print STDERR "Running command: $command\n";
my $pid = open(BOWTIE, "-|", "stdbuf -oL $command");
#my $pid = open(BOWTIE, "-|", "stdbuf -oL bowtie2 -q --sensitive -a -p $cpus --no-mixed --reorder -x $bowtie_db -U $fastq_file1 2> $bowtie_out");
